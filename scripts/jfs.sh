...
function run_solver(){
  ...
  do
    ...
    start_time=$(date +%s.%N)
    solving_results=$(timeout "$timeout_seconds" /root/jfs/build/bin/jfs "$smt_file_path" 2>&1) # file path of jfs
    return_code=$?
    end_time=$(date +%s.%N)
...
run_solver "jfs-benchmark.csv" "results/jfs/jfs.csv"
run_solver "our-benchmark.csv" "results/our/jfs.csv"
