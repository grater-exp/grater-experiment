...
# 创建存储文件夹（如果不存在）
mkdir -p /root/output/output-500-48h/jfs

# 指定输出的结果文件路径
result_file="/root/output/output-500-48h/jfs/result-48h.csv"
...
# 指定分类文件
select_files="$experiment_dir/select-48h-jfs.csv"

mkdir -p /root/scripts/tmp-500
cd /root/scripts/tmp-500
...
function run_solver(){
  ...
  do
    ...
    start_time=$(date +%s.%N)
    solving_results=$(timeout "$timeout_seconds" /root/jfs/build/bin/jfs "$smt_file_path" 2>&1)
    return_code=$?
    end_time=$(date +%s.%N)
