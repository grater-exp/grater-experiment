...
# 创建存储文件夹（如果不存在）
mkdir -p /root/output/output-500-9000s/bitwuzla

# 指定输出的结果文件路径
result_file="/root/output/output-500-9000s/bitwuzla/result-9000s.csv"
...
# 指定分类文件
small_files="$experiment_dir/small.csv"
medium_files="$experiment_dir/medium.csv"
large_files="$experiment_dir/large.csv"
...
function run_solver(){
  ...
  do
    ...
    start_time=$(date +%s.%N)
    solving_results=$(timeout "$timeout_seconds" bitwuzla "$smt_file_path" 2>&1)
    return_code=$?
    end_time=$(date +%s.%N)
...
# 调用函数
run_solver $small_files
run_solver $medium_files
run_solver $large_files

echo "任务完成，结果已写入 $result_file"
